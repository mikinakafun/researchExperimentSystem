...
function generationMetadata(overrides = {}) {
  return {
    ...
    source: 'generated', attempts: 1, diagnostics: { rejections: [] }, ...overrides,
  };
}
...
function resultPayload(overrides = {}) {
  ...
  return {
    sessionId: 'offline-session', recordType: 'participant', language: 'ja', condition: 'standard',
    fragment: '友人と公園を歩いた。',
    questions: Array.from({ length: 6 }, (_, index) => `質問${index + 1}？`),
    answers: Array.from({ length: 6 }, (_, index) => `回答${index + 1}。`),
    questionMetadata: Array.from({ length: 6 }, () => ({ conditionFocus: 'standard' })),
