...
test('incomplete or inconsistent generation diagnostics cannot be silently saved', () => {
  ...
  for (const bad of [
    ...
    { ...good, attempts: 2, diagnostics: { rejections: [{ attempt: 2, stage: 'candidate', flags: ['rejected'] }] } },
    { ...good, attempts: 3, diagnostics: { rejections: [{ attempt: 1, stage: 'candidate', flags: ['rejected'] }, { attempt: 3, stage: 'candidate', flags: ['rejected'] }] } },
