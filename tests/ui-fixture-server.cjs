  ...
  if (req.url.startsWith('/api/')) {
    try {
      ...
      if (req.url === '/api/follow-up') {
        ...
        return json(res, 200, { ...fallbackQuestion(body), language: body.language, promptVersion: PROMPT_CONFIG.followUpVersion, source: 'fallback', model: 'fallback', requestId: null, attempts: 3, settings: { temperature: 0.55, candidateCount: 1, maxAttempts: 3 }, diagnostics: { rejections: [1, 2, 3].map((attempt) => ({ attempt, flags: ['offline_fixture'] })) } });
      }
      if (req.url === '/api/narrative') {
        ...
        return json(res, 200, { language: body.language, narrative: joinNarrative(sentences, body.language), sentences, promptVersion: PROMPT_CONFIG.version, source: 'generated', model: 'offline-fixture', requestId: 'offline-narrative', attempts: 1, settings: { temperature: 0.75, candidateCount: 1, maxAttempts: 3 }, diagnostics: { rejections: [] } });
      }
