...
`core/DESIGN.ja.md`（旧`docs/protocol/PROTOCOL.md`は2026-09-07に退役）とPrompt Catalog（質問v0.4.4・文章v0.4.3）を参照した、参加者フロー確認用のローカルmockです。質問生成と最終文章生成に実際のOpenAI Responses APIを使用できます。

現行の質問生成版は `prompts/v0.4.4-mock-draft/` です（DEC-039）。ターン別の推奨順序を外し、初期断片と全質問・回答から対象ごとの既知・不明・既質問の内容を読み、条件の焦点内で次の一問を選びます。部分的に思い出せる内容は利用し、不明な対象の追及を避けます。文章生成には `v0.4.3-mock-draft/narrative.{ja,en}.txt` を使用し、入力を素材とする創作を許可します（DEC-036）。主仮説と主対比を維持し、質問から創作を含む読後反応までの処理全体を比較します。
質問の長さ、疑問符数、冒頭の定型、必須語、証拠中の語彙、ターン機能の順序を採用条件にはしません。回答状態の判断は生成モデルへの指示であり、別の状態判定器や追加API呼び出しは設けません。条件外の語彙チェックは補助的なものであり、意味的な条件適合や回答状態の判断精度を保証しません。詳細はDEC-035・039と[質問生成仕様v0.4.4](docs/prompts/PROMPT_CATALOG_V0.4.4_DRAFT.md)を参照してください。
...
- 新しい質問の`promptVersion`は`prompt-catalog-v0.4.4-mock-draft`、文章は`prompt-catalog-v0.4.3-mock-draft`です。保存先とschema v2は維持するため、旧質問版との比較・集計では質問の生成記録にある版で区別してください。既存結果を移行・書き換えしません。
