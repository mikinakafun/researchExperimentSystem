...
実行時に使うプロンプト本文は、コードに埋め込まずこのディレクトリから読み込みます。質問本文は `v0.4.5-mock-draft/`、文章本文は `v0.4.3-mock-draft/narrative.{ja,en}.txt` が唯一の正本です。質問は対象ごとの回答状態を優先して選び、文章は入力を素材とする創作を許します。
...
- Follow-up instructions and guidance: `v0.4.5-mock-draft/follow-up.{ja,en}.txt` and `follow-up-guidance.{ja,en}.txt`.
