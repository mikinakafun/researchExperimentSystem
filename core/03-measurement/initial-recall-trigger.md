...
### English version（本実験用。パイロット版では使用しない —追記）
...
| 言語 | パイロット版は日本語のみ。本実験では開始時に日本語または英語を選択し、セッション中は混在させない |
...
2. 一文・100文字の初期断片で、6ターンの生成質問の入力として十分な情報が得られるかをパイロットで確認し、上限値を正式確定する。
...
日本語が正本である。英語版は独立に書き起こさず、**必要になった時点でこの日本語から生成する**。逐語訳ではなく意味を合わせた研究版とし（18項目の評価尺度と同じ方針）、項目IDは一切変えない。生成した英語版は日本語版と同じレビューを要する。二つの別文書として分岐させないこと。
