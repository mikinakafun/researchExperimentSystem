...
const CONDITIONS = (process.env.BATCH_CONDITIONS?.split(",").map((value) => value.trim()).filter(Boolean).length
  ? process.env.BATCH_CONDITIONS.split(",").map((value) => value.trim()).filter(Boolean)
  : ["standard", "visual", "odor"]);
const today = new Date().toISOString().slice(0, 10).replaceAll("-", "");
const BATCH_ID = process.env.BATCH_ID || `batch-${today}-persona`;
const DATA_DIRECTORY = process.env.BATCH_OUTPUT_DIR?.trim() || path.join(process.cwd(), "data");
const LOG_PATH = path.join(DATA_DIRECTORY, "persona-batch-log.jsonl");
const RAW_PATH = path.join(DATA_DIRECTORY, "api-responses.jsonl");
...
async function postJson(pathname, body) {
  ...
  for (let attempt = 1; attempt <= 3; attempt += 1) {
    try {
      ...
      await appendFile(RAW_PATH, `${JSON.stringify({ at: new Date().toISOString(), kind: "route", pathname, attempt, request: body, response: payload, status: response.status })}\n`, "utf8");
...
async function requestSimulatedAnswer(persona, question, retryReason = "") {
  ...
  const payload = await response.json().catch(() => null);
  await mkdir(DATA_DIRECTORY, { recursive: true });
  await appendFile(RAW_PATH, `${JSON.stringify({ at: new Date().toISOString(), kind: "simulated_answer", personaId: persona.id, attemptReason: retryReason, request: { question }, response: payload, status: response.status })}\n`, "utf8");
